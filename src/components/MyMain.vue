        ...
        <div v-if='searchedFilms' class="films-container">
          <div v-for="film, index in searchedFilms" :key="index" class="card">
            ...
            <div>
              ...
              <span><i v-for="n in getVote(film.vote_average)"
                :key="'fullStar' + n" class="fa-solid fa-star"></i><i
                  v-for="n in 5 - getVote(film.vote_average)"
                  :key="'emptyStar' + n"
                  class="fa-regular fa-star"></i></span>
              <country-flag v-if="film.original_language"
                :country="!film.original_language ? ``
                : `${film.original_language}`" size='normal' />
            </div>
          </div>
          <div v-for="series, index in searchedSeries" :key="index" class="card">
            ...
            <div>
              ...
              <span>{{ series.name }}</span>
              <div><i v-for="n in getVote(series.vote_average)"
                 :key="'fullStar' + n" class="fa-solid fa-star"></i><i
                  v-for="n in 5 - getVote(series.vote_average)"
                  :key="'emptyStar' + n" class="fa-regular fa-star"></i>
              </div>
              <country-flag v-if="series.original_language"
                :country="!series.original_language ? ``
                : `${series.original_language}`" size='normal' />
            </div>
          </div>
        </div>
      </div>
